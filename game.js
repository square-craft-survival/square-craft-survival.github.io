...
console.log("SQUARE CRAFT SURVIVAL - TOOLS + HOLD-TO-MINE");
...
// Mining progress UI
const miningHud = document.createElement("div");
Object.assign(miningHud.style, {
    position: "fixed",
    left: "50%",
    top: "56%",
    transform: "translateX(-50%)",
...
Object.assign(miningFill.style, {
    width: "0%",
    height: "100%",
    background: "white"
});
...
const hotbarItems = [null, null, null, null, null, null, null];
...
const recipes = [
    {
        name: "Crafting Wood",
        input: { wood: 1 },
        output: { craftingWood: 5 },
        description: "1 Wood → 5 Crafting Wood"
    },
    {
        name: "Sticks",
        input: { craftingWood: 2 },
        output: { sticks: 4 },
        description: "2 Crafting Wood → 4 Sticks"
    },
    {
        name: "Axe",
        input: { craftingWood: 3, sticks: 2 },
        output: { axe: 1 },
        description: "3 Crafting Wood + 2 Sticks → Axe"
    },
    {
        name: "Pickaxe",
        input: { craftingWood: 3, sticks: 2 },
        output: { pickaxe: 1 },
        description: "3 Crafting Wood + 2 Sticks → Pickaxe"
    }
];
...
function getMiningTime(blockType) {
    ...
    if (blockType === "bedrock") return Infinity;
    if (blockType === "leaves") return 0.5;
    if (blockType === "grass" || blockType === "dirt") return 1;

    // Raw wood and Crafting Wood are both wood-like.
    if (blockType === "wood" || blockType === "craftingWood") {
        return tool === "axe" ? 1 : 3;
    }

    if (blockType === "stone") {
        return tool === "pickaxe" ? 3 : 5;
    }
...
function getChunkForWorld(x, z) {
    return loadedChunks.get(
        chunkKey(worldToChunk(x), worldToChunk(z))
    ) || null;
}

// =====================================================
// WORLD RANDOM / TERRAIN
// =====================================================
...
function getTerrainHeight(x, z) {
    const huge = Math.sin(x * 0.026) * 2.6 + Math.cos(z * 0.024) * 2.3;
    const medium = Math.sin((x + z) * 0.052) * 1.7 + Math.cos((x - z) * 0.041) * 1.3;
    const small = Math.sin(x * 0.11) * 0.7 + Math.cos(z * 0.10) * 0.7;

    return Math.max(0, Math.min(11, Math.floor(4 + huge + medium + small)));
}
...
function canTreeSpawn(x, z) {
    ...
    if (coordinateRandom(x, z) >= 0.027) return false;
...
function getTreeBlocks(treeX, treeZ) {
    if (!canTreeSpawn(treeX, treeZ)) return [];

    const groundY = getTerrainHeight(treeX, treeZ);
    const trunkHeight = coordinateRandom(treeX + 500, treeZ + 500) > 0.5 ? 5 : 4;
    const blocks = [];
...
function setWorldEdit(x, y, z, typeOrNull) {
    const ck = chunkKey(worldToChunk(x), worldToChunk(z));

    if (!chunkEdits.has(ck)) {
        chunkEdits.set(ck, new Map());
    }

    chunkEdits.get(ck).set(blockKey(x, y, z), typeOrNull);
}
...
function generateChunkBlocks(cx, cz) {
    ...
    for (let x = minX; x <= maxX; x++) {
        for (let z = minZ; z <= maxZ; z++) {
            ...
            putBlock(x, height, z, "grass");
            putBlock(x, height - 1, z, "dirt");
            putBlock(x, height - 2, z, "dirt");

            for (let y = height - 3; y > bedrockY; y--) {
                putBlock(x, y, z, "stone");
            }

            putBlock(x, bedrockY, z, "bedrock");
        }
    }

    // Check a little beyond the border because leaves cross chunk boundaries.
    for (let treeX = minX - 2; treeX <= maxX + 2; treeX++) {
        for (let treeZ = minZ - 2; treeZ <= maxZ + 2; treeZ++) {
            for (const block of getTreeBlocks(treeX, treeZ)) {
                putBlock(block.x, block.y, block.z, block.type);
            }
        }
    }
    ...
    if (edits) {
        for (const [key, typeOrNull] of edits) {
            if (typeOrNull === null) {
                blocks.delete(key);
                continue;
            }
            ...
            blocks.set(key, { x, y, z, type: typeOrNull });
        }
    }
...
function createGrassTopTexture() {
    ...
    ctx.fillStyle = "#4b9d38";
    ctx.fillRect(0, 0, 16, 16);

    const colors = ["#5bb347", "#3f8730", "#67bc4e", "#36792b"];
...
function createDirtTexture() {
    ...
    const colors = ["#85593a", "#5c3a24", "#986641", "#67412a"];

    for (let i = 0; i < 54; i++) {
        const x = (i * 3 + i * i * 2) % 16;
        const y = (i * 7 + 5) % 16;
        ctx.fillStyle = colors[i % colors.length];
        ctx.fillRect(x, y, 1, 1);
    }
...
function createWoodSideTexture() {
    ...
    const stripes = ["#4f311d", "#7c5434", "#5b3922"];
...
function createLeavesTexture() {
    ...
    ctx.fillStyle = "#2e6d2c";
    ctx.fillRect(0, 0, 16, 16);

    const colors = ["#3d8538", "#245822", "#4a9141", "#326f2e"];
...
function createStoneTexture() {
    ...
    const colors = ["#7d7d7d", "#555555", "#8c8c8c", "#606060"];
...
function createBedrockTexture() {
    ...
    const colors = ["#111111", "#454545", "#333333", "#5a5a5a"];

    for (let i = 0; i < 82; i++) {
        const x = (i * 11 + i * i * 3) % 16;
        const y = (i * 7 + 4) % 16;
        ctx.fillStyle = colors[i % colors.length];
        ctx.fillRect(x, y, 1, 1);
    }
...
function createMaterials() {
    const grassTop = new THREE.MeshLambertMaterial({ map: createGrassTopTexture() });
    const grassSide = new THREE.MeshLambertMaterial({ map: createGrassSideTexture() });
    const dirt = new THREE.MeshLambertMaterial({ map: createDirtTexture() });
    const woodSide = new THREE.MeshLambertMaterial({ map: createWoodSideTexture() });
    const woodTop = new THREE.MeshLambertMaterial({ map: createWoodTopTexture() });
    const leaves = new THREE.MeshLambertMaterial({ map: createLeavesTexture() });
    const stone = new THREE.MeshLambertMaterial({ map: createStoneTexture() });
    const craftingWood = new THREE.MeshLambertMaterial({ map: createCraftingWoodTexture() });
    const bedrock = new THREE.MeshLambertMaterial({ map: createBedrockTexture() });
...
function getBlockTint(block) {
    const random = coordinateRandom(block.x * 7 + block.y * 3, block.z * 11);
...
function rebuildChunk(chunk) {
    ...
    const grouped = {};
    for (const type of allBlockTypes) grouped[type] = [];
    ...
    for (const type of allBlockTypes) {
        ...
        for (let i = 0; i < blocks.length; i++) {
            ...
            mesh.setColorAt(i, getBlockTint(block));
        }
...
function refreshLoadedChunks(force = false) {
    if (!camera) return;

    const playerCX = worldToChunk(camera.position.x);
    const playerCZ = worldToChunk(camera.position.z);

    if (
        !force &&
        playerCX === lastPlayerChunkX &&
        playerCZ === lastPlayerChunkZ
    ) {
        return;
    }
    ...
    const radius = LOAD_DISTANCE + 0.35;
...
function getLoadedBlock(x, y, z) {
    const chunk = getChunkForWorld(x, z);
    if (!chunk) return null;
    return chunk.blocks.get(blockKey(x, y, z)) || null;
}

function blockExists(x, y, z) {
    return getLoadedBlock(x, y, z) !== null;
}

function removeLoadedBlock(x, y, z) {
    const chunk = getChunkForWorld(x, z);
    if (!chunk) return null;
    ...
    if (!block || block.type === "bedrock") return null;

    chunk.blocks.delete(key);
    setWorldEdit(x, y, z, null);
    rebuildChunk(chunk);

    return block;
}

function addLoadedBlock(x, y, z, type) {
    const chunk = loadChunk(worldToChunk(x), worldToChunk(z));
    const key = blockKey(x, y, z);

    if (chunk.blocks.has(key)) return false;

    chunk.blocks.set(key, { x, y, z, type });
    setWorldEdit(x, y, z, type);
    rebuildChunk(chunk);
...
function getIconBackground(type) {
    switch (type) {
        ...
        case "sticks":
            return "linear-gradient(70deg, transparent 0 36%, #6e4729 36% 47%, #9a6a40 47% 55%, #6e4729 55% 64%, transparent 64% 100%)";

        case "axe":
            return "linear-gradient(65deg, transparent 0 34%, #775034 34% 42%, #9a6a40 42% 48%, #646464 48% 62%, #8a8a8a 62% 72%, transparent 72% 100%)";

        case "pickaxe":
            return "linear-gradient(90deg, transparent 0 15%, #777 15% 80%, transparent 80% 100%), linear-gradient(65deg, transparent 0 45%, #7a5131 45% 54%, transparent 54% 100%)";
...
function addItemToInventory(type, amount = 1) {
    ...
    if (!hotbarItems.includes(type)) {
        const emptyIndex = hotbarItems.indexOf(null);

        if (emptyIndex !== -1) {
            hotbarItems[emptyIndex] = type;
        }
    }
...
function removeItemFromInventory(type, amount = 1) {
    if (!Object.prototype.hasOwnProperty.call(inventory, type)) return false;
...
function updateHotbar() {
    for (let i = 0; i < hotbarSlots.length; i++) {
        ...
        slot.title = `${itemName(type)}: ${inventory[type]}`;
    }

    const type = hotbarItems[selectedHotbarIndex];
...
Object.assign(craftingOverlay.style, {
    display: "none",
    position: "fixed",
    inset: "0",
...
Object.assign(craftingPanel.style, {
...
function craftRecipe(recipe) {
    ...
    for (const type of Object.keys(recipe.output)) {
        if (!hotbarItems.includes(type)) {
            const emptyIndex =
                hotbarItems.indexOf(null);

            if (emptyIndex !== -1) {
                hotbarItems[emptyIndex] =
                    type;
            }
        }
    }
...
function updateCraftingMenu() {
    ...
    Object.assign(recipesTitle.style, {
        fontSize: "21px",
        marginBottom: "10px"
    });
    ...
    for (const recipe of recipes) {
        ...
        Object.assign(recipeName.style, {
            fontSize: "20px",
            fontWeight: "bold",
            marginBottom: "4px"
        });
